...
func (h *Handler) CreateSong(c *fiber.Ctx) error {
	...
	// h.log.Debug("External API url", externalApiUrl)
	...
	// 	h.log.Error("Not OK", slog.String(fmt.Sprint("External api return status code: %d", resp.StatusCode)))
...
func (h *Handler) UpdateSongInfo(c *fiber.Ctx) error {
	...
	res, err := repository.DB.Exec(`UPDATE song SET song = $1, "group" = $2 WHERE id = $3 RETURNING id`, song.Song, song.Text, id)
	if err != nil {
